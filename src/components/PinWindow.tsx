...
const PinWindow: React.FC<PinWindowProps> = ({ year, bce, lat, lng, visible }) => {
  ...
  useEffect(() => {
    ...
    const publish = async () => {
      const eventData = (document.getElementById('eventTitle') as HTMLInputElement).value
      if (!eventData) {
        console.error('Event title is required')
        return
      }

      const yearText = (document.getElementById('inputYear') as HTMLInputElement).textContent?.split(' ')
      if (!yearText) return
      ...
      const data = {
        title: eventData,
        header: (document.getElementById('header') as HTMLInputElement).value,
...
export const createPinWindow = (year: number, bce: string, lat: number, lng: number) => {
//   const pinWindow = PinWindow(year, bce, lat, lng)
  return <PinWindow year={year} bce={bce} lat={lat} lng={lng} visible={true} />
}
